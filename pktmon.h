...
typedef HRESULT (WINAPI *PacketMonitorAddCaptureConstraint_t)(
    _In_ PACKETMONITOR_SESSION session,
    _In_ PACKETMONITOR_PROTOCOL_CONSTRAINT const* captureConstraint
);

typedef HRESULT (WINAPI *PacketMonitorAddSingleDataSourceToSession_t)(
    _In_ PACKETMONITOR_SESSION session,
    _In_ PACKETMONITOR_DATA_SOURCE_SPECIFICATION const* dataSource
);

typedef HRESULT (WINAPI *PacketMonitorAttachOutputToSession_t)(
    _In_ PACKETMONITOR_SESSION session,
    _In_ VOID* outputHandle
);

typedef VOID (WINAPI *PacketMonitorCloseRealtimeStream_t)(
    _In_ PACKETMONITOR_REALTIME_STREAM realtimeStream
);

typedef VOID (WINAPI *PacketMonitorCloseSessionHandle_t)(
    _In_ PACKETMONITOR_SESSION session
);

typedef HRESULT (WINAPI *PacketMonitorCreateLiveSession_t)(
    _In_ PACKETMONITOR_HANDLE handle,
    _In_ PCWSTR name,
    _Out_ PACKETMONITOR_SESSION* session
);

typedef HRESULT (WINAPI *PacketMonitorCreateRealtimeStream_t)(
    _In_ PACKETMONITOR_HANDLE handle,
    _In_ PACKETMONITOR_REALTIME_STREAM_CONFIGURATION const* configuration,
    _Out_ PACKETMONITOR_REALTIME_STREAM* realtimeStream
);

typedef HRESULT (WINAPI *PacketMonitorEnumDataSources_t)(
    _In_ PACKETMONITOR_HANDLE handle,
...
typedef HRESULT (WINAPI *PacketMonitorInitialize_t) (
    _In_ UINT32 apiVersion,
    _Reserved_ void* reserved,
    _Out_ PACKETMONITOR_HANDLE* handle
);

typedef HRESULT (WINAPI *PacketMonitorSetSessionActive_t)(
    _In_ PACKETMONITOR_SESSION session,
    _In_ BOOLEAN active
);

typedef VOID (WINAPI *PacketMonitorUninitialize_t) (
    _In_ PACKETMONITOR_HANDLE handle
);

/*
    --allowlist-type PacketMonitorAddCaptureConstraint_t \
    --allowlist-type PacketMonitorAddSingleDataSourceToSession_t \
    --allowlist-type PacketMonitorAttachOutputToSession_t \
    --allowlist-type PacketMonitorCloseRealtimeStream_t \
    --allowlist-type PacketMonitorCloseSessionHandle_t \
    --allowlist-type PacketMonitorCreateLiveSession_t \
    --allowlist-type PacketMonitorCreateRealtimeStream_t \
    --allowlist-type PacketMonitorEnumDataSources_t \
    --allowlist-type PacketMonitorInitialize_t \
    --allowlist-type PacketMonitorSetSessionActive_t \
    --allowlist-type PacketMonitorUninitialize_t  \
    --allowlist-type PACKETMONITOR_STREAM_METADATA  \
    --allowlist-type PKTMON_DIRECTION_TAG \
    --allowlist-type PKTMON_PACKET_TYPE \
    --allowlist-type PKTMON_DROP_LOCATION \
    --allowlist-type PKTMON_DROP_REASON
*/
